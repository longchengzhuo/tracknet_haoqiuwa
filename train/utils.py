...
def train(epoch, model, optimizer, loss_fn, data_loader, input_type, display_step, save_dir, device):
    model.train()
...
def evaluation(model, data_loader, tolerance, input_type, device):
    model.eval()
    data_prob = tqdm(data_loader)
    TP, TN, FP1, FP2, FN = [], [], [], [], []
    for step, (i, x, y, c) in enumerate(data_prob):
