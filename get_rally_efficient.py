    ...
    for i in range(len(startend_row)):
        ...
        if (key_startend_row[i:i+90] == 1).sum() == 1 and startend_row[i] == 1:
            for j in range(i,len(startend_row)):
                # 在100帧内结束的回合数
                if (key_startend_row[i - 100:i + 1] == 0).sum() == 1 and startend_row[i] == 0:
                    correct_rally += 1
                    break
...
if __name__ == "__main__":
    ...
    parser.add_argument("--csv_path", type=str, default='E:/biancheng\get_badminton_rounds/test.csv', help="需处理表格的路径")
